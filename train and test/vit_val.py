...
models_parent_dir = '/root/autodl-tmp/program/model_vit/model_vit/sample/'
...
# /root/autodl-tmp/program/model_vit/model_vit/guassian_sample_results/1
# /root/autodl-tmp/program/model_vit/model_vit/sample_result/1
base_output_dir = '/root/autodl-tmp/program/model_vit/model_vit/sample_result/1'
excel_load = 'vit_confusion.xlsx'
...
class ViT(nn.Module):
    def __init__(self, num_classes=2, pretrained_weight_path=None, target_image_size=16):
        ...
        self.projection = nn.Sequential(
            nn.Conv2d(1280, 32, kernel_size=1),  #  [B, 1280, 7, 7]
            nn.BatchNorm2d(32),
            nn.GELU(),
            nn.AdaptiveAvgPool2d((target_image_size, target_image_size))  #  [B, 32, 16, 16]
        )
...
def test_different_N_values():
    ...
    for model_idx, model_path in enumerate(model_paths):
        ...
        plt.title(f'ViT: Accuracy, Recall, F1 vs Sampling Points\n(Model: {model_folder_name})')
        plt.legend()
        plt.grid(True)
        plt.xticks(N_list)
        plt.ylim(0, 100)
        plt.tight_layout()
        combined_svg_path = os.path.join(model_output_dir, f'ViT_combined_metrics_trend.svg')
        ...
        plt.xlabel('Sampling Points per Side (N)')
        plt.ylabel('Accuracy (%)')
        plt.title(f'ViT: Accuracy vs Sampling Points\n(Model: {model_folder_name})')
        plt.grid(True)
        plt.xticks(N_list)
        plt.ylim(0, 100)
        plt.tight_layout()
        accuracy_svg_path = os.path.join(model_output_dir, f'ViT_accuracy_trend.svg')
        ...
        plt.xlabel('Sampling Points per Side (N)')
        plt.ylabel('Recall (%)')
        plt.title(f'ViT: Recall vs Sampling Points\n(Model: {model_folder_name})')
        plt.grid(True)
        plt.xticks(N_list)
        plt.ylim(0, 100)
        plt.tight_layout()
        recall_svg_path = os.path.join(model_output_dir, f'ViT_recall_trend.svg')
        ...
        plt.xlabel('Sampling Points per Side (N)')
        plt.ylabel('F1 Score (%)')
        plt.title(f'ViT: F1 Score vs Sampling Points\n(Model: {model_folder_name})')
        plt.grid(True)
        plt.xticks(N_list)
        plt.ylim(0, 100)
        plt.tight_layout()
        f1_svg_path = os.path.join(model_output_dir, f'ViT_f1_trend.svg')
